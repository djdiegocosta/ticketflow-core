-- Hardening das operações administrativas e concorrência de check-in.

REVOKE EXECUTE ON FUNCTION public.update_customer(uuid, text, text, text, text, date, public.customer_gender) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.checkin_ticket(text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.confirm_sale_manual(uuid) FROM PUBLIC, anon;
...
REVOKE EXECUTE ON FUNCTION public.set_mp_credentials(text, text, text, text) FROM PUBLIC, anon;
...
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN RAISE EXCEPTION 'Sem permissão: apenas administradores podem confirmar venda manualmente'; END IF;
  UPDATE public.sales SET status='pago', verification_type='manual_admin', updated_at=now()
...
BEGIN
 ...
 IF _client_identifier IS NOT NULL THEN
   ...
   ON CONFLICT(identifier) DO UPDATE SET attempt_count=CASE WHEN public.checkout_rate_limits.window_start<v_now-interval '10 minutes' THEN 1 ELSE public.checkout_rate_limits.attempt_count+1 END,
 ...
 SELECT b.price INTO v_unit_price FROM public.ticket_batches b JOIN public.events e ON e.id=b.event_id
 ...
 SELECT coalesce(o.pending_sale_expiration_minutes,30) INTO v_expiration_minutes FROM public.organizations o JOIN public.events e ON e.organization_id=o.id WHERE e.id=_event_id;
...
BEGIN
 IF NOT (public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'colaborador') OR public.has_role(auth.uid(),'operador_checkin')) THEN RAISE EXCEPTION 'Sem permissão para realizar check-in'; END IF;
 SELECT t.*,e.title AS ev_title,b.name AS b_name INTO v_ticket FROM public.tickets t JOIN public.sales s ON s.id=t.sale_id JOIN public.events e ON e.id=s.event_id JOIN public.ticket_batches b ON b.id=t.batch_id WHERE t.ticket_code=_ticket_code;
 IF v_ticket.id IS NULL THEN INSERT INTO public.checkin_log(ticket_code,result,checked_by) VALUES(_ticket_code,'nao_encontrado',auth.uid()); RETURN QUERY SELECT NULL::uuid,NULL::text,NULL::text,NULL::text,'nao_encontrado'::text; RETURN; END IF;
 IF v_ticket.status='cancelado' THEN INSERT INTO public.checkin_log(ticket_id,ticket_code,result,checked_by) VALUES(v_ticket.id,_ticket_code,'cancelado',auth.uid()); RETURN QUERY SELECT v_ticket.id,v_ticket.participant_name,v_ticket.ev_title,v_ticket.b_name,'cancelado'::text; RETURN; END IF;
 IF v_ticket.status='utilizado' THEN INSERT INTO public.checkin_log(ticket_id,ticket_code,result,checked_by) VALUES(v_ticket.id,_ticket_code,'ja_utilizado',auth.uid()); RETURN QUERY SELECT v_ticket.id,v_ticket.participant_name,v_ticket.ev_title,v_ticket.b_name,'ja_utilizado'::text; RETURN; END IF;
 ...
 IF NOT FOUND THEN INSERT INTO public.checkin_log(ticket_id,ticket_code,result,checked_by) VALUES(v_ticket.id,_ticket_code,'ja_utilizado',auth.uid()); RETURN QUERY SELECT v_ticket.id,v_ticket.participant_name,v_ticket.ev_title,v_ticket.b_name,'ja_utilizado'::text; RETURN; END IF;
