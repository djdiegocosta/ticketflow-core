...
-- Correção da constraint de conflito em checkout_abandonments
ALTER TABLE public.checkout_abandonments DROP CONSTRAINT IF EXISTS checkout_abandonments_event_id_buyer_whatsapp_key;
CREATE UNIQUE INDEX IF NOT EXISTS checkout_abandonments_event_id_buyer_whatsapp_idx ON public.checkout_abandonments (event_id, buyer_whatsapp);

-- Ajuste na RPC create_pending_sale para usar a nova indexação única
...
-- Ajuste na track_checkout_abandonment
