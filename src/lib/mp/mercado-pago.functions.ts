...
export const saveMpCredentials = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: existing } = await supabaseAdmin.from("mp_config").select("public_key, access_token_encrypted, webhook_secret_encrypted").eq("organization_id", data.organization_id).eq("environment", data.environment).maybeSingle();
    ...
    const { error } = await supabaseAdmin.from("mp_config").upsert({ organization_id: data.organization_id, environment: data.environment, public_key: publicKey || existing?.public_key || "", access_token_encrypted: encryptedToken, webhook_secret_encrypted: encryptedWebhookSecret, updated_at: new Date().toISOString() }, { onConflict: "organization_id,environment" });
...
export const validateMpCredentials = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabaseAdmin.from("mp_config").update({ validated_at: new Date().toISOString() }).eq("organization_id", data.organization_id).eq("environment", data.environment);
    return { success: true };
  });
...
export const createMpPix = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const mpRes = await fetch("https://api.mercadopago.com/v1/payments", { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json", "X-Idempotency-Key": sale.id }, body: JSON.stringify({ transaction_amount: sale.total_amount, description: `Ingresso TicketFlow - Venda ${sale.sale_code}`, payment_method_id: "pix", external_reference: sale.id, notification_url: notificationUrl, payer: { email: sale.buyer_email, first_name: sale.buyer_name.split(" ")[0], last_name: sale.buyer_name.split(" ").slice(1).join(" ") || "Cliente" } }) });
    ...
    await supabaseAdmin.from("sales").update({ mp_payment_id: mpPaymentId, mp_qr_code: qrCode, mp_qr_code_base64: qrCodeBase64 }).eq("id", data.sale_id).eq("status", "pendente");
