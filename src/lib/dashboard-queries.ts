...
/* --- Temperatura do evento (velocidade de vendas por hora) --------------- */

/** Limites configuráveis (vendas/hora). Isolado para configuração futura. */
...
export function classifyTemperature(
  ...
  if (salesPerHour >= thresholds.explodindo) return "explodindo";
  if (salesPerHour >= thresholds.quente) return "quente";
  if (salesPerHour >= thresholds.aquecendo) return "aquecendo";
  return "normal";
}

/** Vendas por hora nas últimas 24h, a partir da lista de vendas já carregada. */
export function salesVelocity(
  sales: { created_at: string; status: string; is_courtesy?: boolean }[],
  hours = 24,
) {
  const since = Date.now() - hours * 3600_000;
  const recent = sales.filter(
    (s) => !s.is_courtesy && s.status === "pago" && new Date(s.created_at).getTime() >= since,
  );
...
export function useAudienceStats() {
  return useQuery({
    queryKey: ["customers", "audience-stats"],
    queryFn: async () => {
      const [{ data: customers, error }, { data: paidSales, error: salesError }] = await Promise.all([
        supabase.from("customers").select("id, data_nascimento, cidade, created_at"),
        supabase.from("sales").select("customer_id").eq("status", "pago"),
      ]);
      if (error) throw error;
      ...
      (paidSales ?? []).forEach((s) => {
        if (s.customer_id) purchases.set(s.customer_id, (purchases.get(s.customer_id) ?? 0) + 1);
      });
      ...
      let topBracket: string | null = null;
      let topBracketCount = 0;
      brackets.forEach((b) => {
        const count = ages.filter((a) => a >= b.min && a <= b.max).length;
        if (count > topBracketCount) {
          topBracketCount = count;
          topBracket = b.label;
        }
      });
