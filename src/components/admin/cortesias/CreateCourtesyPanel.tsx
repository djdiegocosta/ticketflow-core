...
import { Upload, FileText, Keyboard, AlertCircle, Trash2, Loader2 } from "lucide-react";
...
export function CreateCourtesyPanel({ open, onOpenChange, onSuccess }: CreateCourtesyPanelProps) {
  ...
  const handleSubmit = async () => {
    if (!selectedEvent || !selectedBatch) {
      toast.error("Selecione um evento e um lote de cortesias primeiro.");
      return;
    }
    if (validCount === 0) {
    ...
    try {
      ...
      toast.error(`Erro ao emitir cortesias: ${err?.message ?? "erro desconhecido"}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClose = (open: boolean) => {
    if (!open && isDirty && participants.length > 0) {
      if (!confirm("Existem dados preenchidos. Deseja realmente sair?")) return;
      setParticipants([]);
      setSingleName("");
      setBulkText("");
      setActiveTab("type");
