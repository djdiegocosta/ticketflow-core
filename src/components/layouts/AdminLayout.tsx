...
import { BarChart3, CalendarDays, Gift, LayoutDashboard, Receipt, Settings, ShieldCheck, Ticket, Users, UsersRound, Wrench, Menu, X, LogOut, Sun, Moon } from "lucide-react";
...
function AdminLayoutContent() {
  ...
  return <div className="flex h-screen overflow-hidden bg-[var(--bg-primary)]">
    ...
    <aside className={["flex h-full w-60 shrink-0 flex-col border-r border-[var(--border-subtle)] bg-[var(--bg-secondary)] transition-transform duration-300", isMobile ? "fixed left-0 top-0 z-50 transform" : "relative translate-x-0", isMobile && !isSidebarOpen ? "-translate-x-full" : "translate-x-0"].join(" ")}>
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden p-3 pb-6 scrollbar-thin"><div className="flex flex-col gap-1">{filteredMenu.map((item) => { const active = isActive(item.to, item.exact); return <Link key={item.to} to={item.to} className={["flex items-center gap-3 rounded-none border-l-[3px] px-3 py-2 text-body transition-colors", active ? "border-[var(--accent)] bg-[var(--accent-muted)] text-[var(--accent-text)] dark:border-l-4" : "border-transparent text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)] hover:text-[var(--text-primary)]"].join(" ")}><item.icon className="h-4 w-4" /><span>{item.label}</span></Link>; })}</div></nav>
      <div className="shrink-0 border-t border-[var(--border-subtle)] p-3"><button type="button" onClick={toggleTheme} className="flex w-full items-center gap-3 rounded-[var(--radius-sm)] px-3 py-2 text-body text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-tertiary)] hover:text-[var(--text-primary)]">{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}<span>{theme === "dark" ? "Tema claro" : "Tema escuro"}</span></button><button type="button" onClick={logout} className="mt-1 flex w-full items-center gap-3 rounded-[var(--radius-sm)] px-3 py-2 text-body text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-tertiary)] hover:text-[var(--error)]"><LogOut className="h-4 w-4" /><span>Sair</span></button></div>
    </aside>
