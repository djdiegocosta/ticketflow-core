    ...
    <div className="flex flex-1 items-end justify-between gap-3">
      ...
      {gaugeValue !== undefined && (
        <div className="relative h-[50px] w-[90px] shrink-0">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-end justify-center"><span className="text-heading-2 font-semibold leading-none text-text-primary">{gaugeValue}%</span></div>
        </div>
      )}
    </div>
  </div>
...
const Panel = ({ title, children, action }: { title: string; children: ReactNode; action?: ReactNode }) => (
  <section className="flex h-full flex-col rounded-[var(--radius-md)] border border-border-subtle bg-bg-secondary p-6">
    <div className="mb-5 flex items-center justify-between gap-4"><h2 className="text-heading-2 text-text-primary">{title}</h2>{action}</div>
    <div className="flex-1">{children}</div>
  </section>
);

export function AdminDashboard() {
  const { data: events = [], isLoading: eventsLoading, event: operationalEvent } = useOperationalEvent();
  ...
  if (eventsLoading || statsLoading || salesLoading || hourlyLoading) {
    return <div className="flex min-h-[400px] items-center justify-center"><span className="mr-2 animate-spin"><Clock className="h-6 w-6" /></span>Carregando métricas...</div>;
  }
