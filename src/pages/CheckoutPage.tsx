...
const checkoutSchema = z.object({
  ...
  participants: z.array(z.object({ name: z.string().min(1, "Nome do participante obrigatório").refine(isFullName, "Nome completo obrigatório") }))
});
...
export default function CheckoutPage() {
  ...
  const form = useForm<CheckoutFormValues>({
    ...
    defaultValues: { buyerName: '', buyerWhatsApp: '', participants: Array(qty).fill({ name: '' }) }
  });
  ...
  useEffect(() => {
    return () => {
      ...
      if (event?.id && !currentSaleId && !abandonmentTracked.current && data.buyerName.length > 3 && onlyDigits(data.buyerWhatsApp).length >= 10) {
        trackAbandonment({ event_id: event.id, buyer_name: data.buyerName, buyer_whatsapp: data.buyerWhatsApp });
        abandonmentTracked.current = true;
      }
    };
  }, [event?.id, currentSaleId, form, trackAbandonment]);

  useEffect(() => { if (event?.organization_id) setLastVisitedOrg(event.organization_id); }, [event?.organization_id]);
  useEffect(() => { if (event?.id) captureRef(event.id, search.ref); }, [event?.id, search.ref]);
  ...
  const onSubmit = async (values: CheckoutFormValues) => {
    ...
    try {
      ...
      if (user) {
        const { data: customerData } = await supabase.rpc('get_or_create_customer', { _organization_id: event.organization_id });
        if (customerData) customerId = customerData as string;
      }
      ...
      try {
        ...
        setPixData({ qr_code: pixResult.qr_code, qr_code_base64: pixResult.qr_code_base64 });
      } catch (pixErr: any) {
  ...
  const handleSameAsBuyer = (checked: boolean | 'indeterminate') => {
    if (checked === true && qty === 1) form.setValue('participants.0.name', form.getValues('buyerName'));
  };
  const copyPix = () => {
    if (!pixData) return;
  ...
  if (isLoadingEvent) return <MobileLayout showFooter={false}><div className="flex min-h-[60vh] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[var(--accent)]" /></div></MobileLayout>;
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                ...
                {qty === 1 && <div className="flex items-center gap-2"><Checkbox id="same" onCheckedChange={handleSameAsBuyer} /><label htmlFor="same" className="text-xs text-[var(--text-secondary)]">Mesmo do comprador</label></div>}
              </div>
              <div className="space-y-4">
                {fields.map((field, index) => (
                  <div key={field.id} className="space-y-2 rounded-[var(--radius-md)] border border-[var(--border-subtle)] p-3">
                    ...
                    <Input placeholder="Nome Sobrenome" {...form.register(`participants.${index}.name` as const)} onInput={(e) => { const t = e.target as HTMLInputElement; t.value = formatName(t.value); form.setValue(`participants.${index}.name`, t.value, { shouldValidate: true }); }} />
            ...
            <Button type="submit" disabled={isCreatingSale} className="mt-4 h-14 w-full bg-[var(--accent)] text-[#111111] font-bold text-lg hover:bg-[var(--accent-hover)] disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {step === 'payment' && (
          <div className="flex flex-col gap-8 animate-in slide-in-from-right duration-300">
            <div className="flex flex-col items-center gap-4 text-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--accent-muted)] text-[var(--accent)]"><Clock className="h-8 w-8" /></div>
              <div className="flex flex-col gap-1"><h2 className="text-heading-2 font-bold text-[var(--text-primary)]">Aguardando Pagamento</h2><p className="text-small text-[var(--text-secondary)]">O seu Pix expira em <span className="font-mono font-bold text-[var(--accent-text)]">{formatTime(countdown)}</span></p></div>
            </div>
            <div className="flex flex-col items-center gap-6 rounded-[var(--radius-lg)] border-2 border-[var(--accent)] bg-[var(--bg-secondary)] p-6">
              <div className="flex min-h-[232px] min-w-[232px] items-center justify-center rounded-xl bg-white p-4 shadow-sm">
                {pixData ? <img src={`data:image/png;base64,${pixData.qr_code_base64}`} alt="QR Code Pix" className="h-[200px] w-[200px]" /> : <Loader2 className="h-8 w-8 animate-spin text-[var(--accent)]" />}
              </div>
              <div className="flex w-full flex-col gap-3">
                <Button variant="outline" disabled={countdown === 0 || !pixData} className="flex h-12 w-full items-center justify-between border-[var(--border-default)] px-4 disabled:opacity-50" onClick={copyPix}>
                  <span className="truncate pr-4 text-xs font-mono text-[var(--text-secondary)]">{pixData ? pixData.qr_code.substring(0, 30) : "Gerando código..."}...</span>
