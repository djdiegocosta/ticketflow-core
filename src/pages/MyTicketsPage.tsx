...
import { usePublicData, Sale } from '@/lib/public-data';
...
export default function MyTicketsPage() {
  ...
  const { getSaleByCode, sales } = usePublicData();
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    const sale = getSaleByCode(saleCode);
    setSearchResult(sale || null);
    setSearched(true);
  };
    ...
    <MobileLayout headerContent={<div className="text-center font-semibold text-small">Meus Ingressos</div>}>
      <div className="flex flex-col gap-6 px-5 py-8">
        ...
        <form onSubmit={handleSearch} className="flex gap-2">
          ...
          <Button type="submit" className="h-12 bg-[var(--accent)] text-[#111111]">
            Buscar
          </Button>
        </form>
      ...
      <div className="p-4 border-b border-[var(--border-subtle)] bg-[var(--bg-tertiary)]/50">
        <div className="flex justify-between items-start">
          <div className="flex flex-col">
            <h3 className="text-heading-3 font-bold text-[var(--text-primary)]">{sale.event_name}</h3>
            <div className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)] mt-1">
              <Calendar className="h-3 w-3" />
              <span>{new Date(sale.event_date).toLocaleDateString('pt-BR')}</span>
            </div>
          </div>
