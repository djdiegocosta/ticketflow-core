...
function StatusBadge({ sale }: { sale: any }) {
  ...
  return <StatusPill tone={sale.status === "pago" ? "accent" : sale.status === "pendente" ? "warning" : sale.status === "expirado" ? "neutral" : "error"}>
...
export function SalesListPage() {
  ...
  return (
    <div className="space-y-5">
      <FilterBar actions={<div className="flex items-center gap-2">
      ...
      <DataTableShell>
        <DataTable className="min-w-[1000px]">
          <DataTableHeadRow columns={["Cliente", "WhatsApp", "Evento", "Lote", "Ingressos", "Valor", "Status", "Data", "Ações"]} />
          <tbody>
            {pageRows.map((sale) => {
              const canCancel = sale.status !== "cancelado" && sale.status !== "reembolsado" && !sale.is_courtesy;
              return <DataTableRow key={sale.id}>
                <DataTableCell variant="primary"><div className="min-w-0"><Link to="/admin/vendas/$id" params={{ id: sale.id }} className="block truncate hover:text-accent-text">{sale.buyer_name}</Link>{sale.sale_code && <span className="font-mono-token text-text-disabled">{sale.sale_code}</span>}</div></DataTableCell>
