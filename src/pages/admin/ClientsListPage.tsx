...
import { DataTable, DataTableCell, DataTablePagination, DataTableRow, DataTableShell } from "@/components/admin/DataTable";
...
export function ClientsListPage() {
  ...
  return <div className="space-y-5">
    ...
    <DataTableShell className="overflow-hidden rounded-[var(--radius-lg)] border border-border-subtle bg-bg-secondary shadow-[var(--shadow-sm)]"><DataTable className="min-w-[980px]"><thead><tr className="border-b border-border-subtle bg-bg-tertiary text-left">{columns.map((col) => <th key={col.label} className="px-4 py-3 text-small font-medium text-text-secondary">{col.key ? <button type="button" onClick={() => toggleSort(col.key as SortKey)} className={cn("flex items-center gap-1 transition-colors hover:text-text-primary", sortKey === col.key && "text-text-primary")}>{col.label}{sortKey === col.key && (sortDir === "asc" ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />)}</button> : col.label}</th>)}</tr></thead><tbody>
      {pageRows.map((client) => <DataTableRow key={client.id} className={cn("cursor-pointer transition-colors hover:bg-bg-tertiary/60", client.total_tickets >= 10 && "border-l-2 border-l-accent")}><DataTableCell variant="primary" onClick={() => navigate({ to: "/admin/clientes/$id", params: { id: client.id } })}>{client.full_name}</DataTableCell><DataTableCell><span className="flex items-center gap-1">{client.whatsapp}<CopyWhatsapp value={client.whatsapp} /></span></DataTableCell><DataTableCell>{client.age} anos</DataTableCell><DataTableCell>{client.total_events}</DataTableCell><DataTableCell variant="strong">{client.total_tickets}</DataTableCell><DataTableCell>{new Date(client.created_at).toLocaleDateString("pt-BR")}</DataTableCell><DataTableCell>{client.last_event_name || "—"}</DataTableCell><DataTableCell><div className="flex items-center gap-1"><Link to="/admin/clientes/$id" params={{ id: client.id }} aria-label={`Visualizar ${client.full_name}`} title="Visualizar / editar" className="rounded-[var(--radius-sm)] p-1.5 text-text-secondary transition-colors hover:bg-bg-tertiary hover:text-text-primary"><Eye className="h-4 w-4" /></Link><a href={whatsappLink(client.whatsapp)} target="_blank" rel="noreferrer" aria-label={`WhatsApp de ${client.full_name}`} title="WhatsApp" className="rounded-[var(--radius-sm)] p-1.5 text-text-secondary transition-colors hover:bg-bg-tertiary hover:text-accent-text"><MessageCircle className="h-4 w-4" /></a><button type="button" onClick={() => setToDelete(client)} aria-label={`Excluir ${client.full_name}`} title="Excluir" className="rounded-[var(--radius-sm)] p-1.5 text-text-secondary transition-colors hover:bg-bg-tertiary hover:text-error"><Trash2 className="h-4 w-4" /></button></div></DataTableCell></DataTableRow>)}
