...
const CreateCourtesyPanelLazy = lazy(() =>
  import("@/components/admin/cortesias/CreateCourtesyPanel").then((m) => ({
    default: m.CreateCourtesyPanel,
  })),
);
...
export function CourtesiesListPage() {
  ...
  const totalCheckins = filteredData.filter(
    (item: any) => !!item.checked_in_at || item.status === "utilizado",
  ).length;
    ...
    <div className="w-full max-w-full space-y-6 overflow-hidden">
      <MiniMetricGrid className="xl:grid-cols-2">
        <MiniMetricCard
          ...
          subtext={operationalEvent ? `no evento ${operationalEvent.title}` : "em todos os eventos"}
        />
        <MiniMetricCard
          icon={CheckCircle}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="h-9 gap-2 rounded-[var(--radius-sm)] bg-accent px-3 text-body font-semibold text-[var(--accent-foreground)] hover:bg-accent-hover"
                >
      ...
      <DataTableShell>
        <DataTable className={isMobile ? "min-w-full" : "min-w-[720px]"}>
          <DataTableHeadRow
            columns={
              isMobile
                ? ["Convidado", "Data", "Status"]
                : [
                    "Convidado",
                    "Evento",
                    "Data de emissão",
                    "Status",
                    <span className="block text-right">Ações</span>,
                  ]
            }
          />
          <tbody>
              ...
              paginatedData.map((item: any) => {
                ...
                return (
                  <DataTableRow key={item.id}>
                    ...
                    {!isMobile && <DataTableCell>{item.sales?.events?.title || "—"}</DataTableCell>}
                    <DataTableCell variant="muted">
                      {new Date(item.created_at).toLocaleDateString("pt-BR")}
                    </DataTableCell>
                    <DataTableCell>
                      <StatusPill tone={isCheckedIn ? "accent" : "neutral"}>
                        {isCheckedIn ? "Utilizado" : "Não utilizado"}
                      </StatusPill>
                    </DataTableCell>
                    {!isMobile && (
                      <DataTableCell className="text-right">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button
                              type="button"
                              aria-label={`Ações de ${item.participant_name}`}
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={async () => {
                                if (
                                  !window.confirm(`Excluir cortesia de "${item.participant_name}"?`)
                                )
                                  return;
                                setIsDeleting(true);
                                try {
                            ...
                            >
                              {isCheckedIn ? (
                                <>
                                  ...
                                  Não é possível excluir (já utilizada)
                                </>
                              ) : (
                                <>
                                  <Trash2 className="mr-2 h-4 w-4" />
