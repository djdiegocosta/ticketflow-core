...
export function CourtesiesListPage() {
  ...
  return <div className="w-full max-w-full space-y-6 overflow-hidden">
    ...
    <div className="flex min-w-0 items-center gap-2"><FilterSearch value={search} onChange={(v) => { setSearch(v); setCurrentPage(1); }} placeholder="Buscar convidado..." /><div className="flex shrink-0 items-center gap-2"><TooltipProvider><Tooltip><TooltipTrigger asChild><Button variant="outline" size="sm" onClick={handleExportPdf} className="gap-2"><Download className="h-4 w-4" /><span className="hidden sm:inline">PDF</span></Button></TooltipTrigger><TooltipContent>Gerar lista PDF</TooltipContent></Tooltip></TooltipProvider></div></div>
    <DataTableShell><DataTable className={isMobile ? "min-w-full" : "min-w-[720px]"}><DataTableHeadRow columns={isMobile ? ["Convidado", "Data", "Status"] : ["Convidado", "Evento", "Data de emissão", "Status", "Ações"]} /><tbody>
      {paginatedData.length === 0 ? <tr><DataTableCell colSpan={isMobile ? 3 : 5} className="py-10 text-center text-body">Nenhuma cortesia encontrada.</DataTableCell></tr> : paginatedData.map((item: any) => { const isCheckedIn = !!item.checked_in_at || item.status === "utilizado"; return <DataTableRow key={item.id}><DataTableCell variant="primary">{formatName(item.participant_name)}</DataTableCell>{!isMobile && <DataTableCell>{item.sales?.events?.title || "—"}</DataTableCell>}<DataTableCell variant="muted">{new Date(item.created_at).toLocaleDateString("pt-BR")}</DataTableCell><DataTableCell><StatusPill tone={isCheckedIn ? "accent" : "neutral"}>{isCheckedIn ? "Utilizado" : "Não utilizado"}</StatusPill></DataTableCell>{!isMobile && <DataTableCell><div className="flex items-center gap-2"><button type="button" onClick={() => { setEditingTicket(item); setNewName(item.participant_name); }} className="p-1 text-text-secondary transition-colors hover:text-accent" title="Editar nome"><Edit2 className="h-4 w-4" /></button><button type="button" disabled={isCheckedIn || isDeleting} onClick={async () => { if (!window.confirm(`Excluir cortesia de "${item.participant_name}"?`)) return; setIsDeleting(true); try { const { error } = await supabase.rpc("delete_courtesy_ticket", { _ticket_id: item.id }); if (error) throw error; toast.success("Cortesia excluída"); queryClient.invalidateQueries({ queryKey: ["tickets", "courtesies"] }); } catch (err: any) { toast.error("Erro ao excluir: " + err.message); } finally { setIsDeleting(false); } }} className="p-1 text-text-secondary transition-colors hover:text-error disabled:cursor-not-allowed disabled:opacity-30" title={isCheckedIn ? "Não é possível excluir cortesia já utilizada" : "Excluir cortesia"}><Trash2 className="h-4 w-4" /></button></div></DataTableCell>}</DataTableRow>; })}
