...
import { Calendar, MapPin, ExternalLink, Edit2, Trash2, CalendarPlus } from "lucide-react";
...
export function EventsListPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* Tabs Filter */}
      <div className="flex items-center gap-1 p-1 bg-[var(--bg-secondary)] rounded-[var(--radius-md)] border border-border-subtle w-fit">
        {["Todos", "Publicados", "Rascunhos", "Encerrados"].map((tab) => (
          <button
            key={tab}
            onClick={() => setFilter(tab)}
            className={cn(
              "px-4 py-2 text-small font-medium rounded-[var(--radius-sm)] transition-all duration-200",
              filter === tab
        ...
        <div className="flex items-center justify-center py-16">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-small text-[var(--text-secondary)]">
              {filter === "Todos" ? "Comece criando seu primeiro evento" : `Nenhum evento com status "${filter}"`}
            </p>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-border-subtle">
            <div className="flex items-center gap-4 text-small text-[var(--text-secondary)]">
              ...
              <span className="hidden sm:inline">
                Mostrando 1–{filteredEvents.length} de {filteredEvents.length}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1">
              <button
                disabled
                className="p-2 border border-border-default text-[var(--text-disabled)] cursor-not-allowed rounded-[var(--radius-sm)]"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                </svg>
              </button>
              <button className="px-3 py-1.5 border border-[var(--accent)] bg-[var(--accent-muted)] text-[var(--accent-text)] text-small font-medium rounded-[var(--radius-sm)]">
                1
              </button>
              <button
                disabled
