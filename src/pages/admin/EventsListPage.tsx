          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            {events.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="group flex flex-col overflow-hidden rounded-[var(--radius-lg)] bg-[var(--bg-primary)] shadow-sm transition-all duration-300 hover:shadow-lg"
                >
                  <div className="relative h-[160px] overflow-hidden">
                    ...
                    <div
                      className={cn(
                        "absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-micro font-medium backdrop-blur-sm",
                        statusStyle.bg,
                        statusStyle.text,
                      )}
                    >
                    ...
                    <div className="absolute right-3 top-3 flex items-center gap-2 opacity-100 transition-opacity duration-200 md:opacity-0 md:group-hover:opacity-100">
                      <TooltipProvider>
                        {isPublished ? (
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <a
                                ...
                                className="rounded-[var(--radius-sm)] bg-[var(--bg-primary)]/90 p-2 text-[var(--text-secondary)] backdrop-blur-sm transition-colors hover:text-[var(--accent)]"
                              >
                                <ExternalLink className="h-4 w-4" />
                              </a>
                            </TooltipTrigger>
                            <TooltipContent side="bottom">
                              <p>Abrir página pública</p>
                            </TooltipContent>
                          </Tooltip>
                        ) : (
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="cursor-not-allowed rounded-[var(--radius-sm)] bg-[var(--bg-primary)]/90 p-2 text-[var(--text-disabled)] backdrop-blur-sm">
                                <ExternalLink className="h-4 w-4" />
                              </div>
                            </TooltipTrigger>
                            <TooltipContent side="bottom">
                              <p>Publique o evento para gerar o link</p>
                            </TooltipContent>
                          </Tooltip>
                        )}
                        <Tooltip>
                  ...
                  <div className="flex flex-1 flex-col gap-4 p-4">
                    <h2 className="line-clamp-2 text-heading-2 text-[var(--text-primary)] transition-colors group-hover:text-[var(--accent)]">
                      {event.title}
                    </h2>
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-small text-[var(--text-secondary)]">
                        <MapPin className="h-4 w-4 text-[var(--text-disabled)]" />
                        <span className="truncate">{event.location}</span>
                      </div>
                    </div>
                    <div className="mt-auto pt-2">
                      <div className="mb-2 flex items-center justify-between text-micro text-[var(--text-secondary)]">
                        <span>
                          {event.sold} de {capacity} ingressos
                        </span>
            ...
            <div className="flex items-center gap-4 text-small text-[var(--text-secondary)]">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="cursor-pointer rounded-[var(--radius-sm)] border border-border-default bg-[var(--bg-secondary)] px-2.5 py-1.5 text-small outline-none focus:border-[var(--accent)]"
                >
              ...
              <span className="hidden sm:inline">
                Mostrando 1–{events.length} de {events.length}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <button
                disabled
                className="cursor-not-allowed rounded-[var(--radius-sm)] border border-border-default p-2 text-[var(--text-disabled)]"
              >
              ...
              <button className="rounded-[var(--radius-sm)] border border-[var(--accent)] bg-[var(--accent-muted)] px-3 py-1.5 text-small font-medium text-[var(--accent-text)]">
                1
              </button>
              <button
                disabled
