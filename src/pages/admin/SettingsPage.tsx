          ...
          {active === "preferencias" && (
            <Panel title="Preferências">
              ...
              <div className="border-t border-[var(--border-subtle)] pt-6">
                <div className="flex items-center gap-2"><Thermometer className="h-4 w-4 text-icon-brand" /><div><p className="text-body text-[var(--text-primary)]">Temperatura do evento</p><p className="text-small text-[var(--text-secondary)]">Classificação baseada em ingressos vendidos (pagos) nas últimas 24 horas.</p></div></div>
                <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <div className="space-y-2"><FieldLabel>Aquecendo a partir de</FieldLabel><div className="flex items-center gap-2"><Input type="number" min={1} value={aquecendo} onChange={(e) => setAquecendo(Number(e.target.value))} className="rounded-[var(--radius-sm)]" /><span className="text-small text-[var(--text-secondary)]">ingressos/dia</span></div></div>
                  <div className="space-y-2"><FieldLabel>Quente a partir de</FieldLabel><div className="flex items-center gap-2"><Input type="number" min={1} value={quente} onChange={(e) => setQuente(Number(e.target.value))} className="rounded-[var(--radius-sm)]" /><span className="text-small text-[var(--text-secondary)]">ingressos/dia</span></div></div>
                  <div className="space-y-2"><FieldLabel>Explodindo a partir de</FieldLabel><div className="flex items-center gap-2"><Input type="number" min={1} value={explodindo} onChange={(e) => setExplodindo(Number(e.target.value))} className="rounded-[var(--radius-sm)]" /><span className="text-small text-[var(--text-secondary)]">ingressos/dia</span></div></div>
                </div>
