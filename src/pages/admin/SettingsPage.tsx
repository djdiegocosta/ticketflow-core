...
export function SettingsPage() {
  ...
  const { data: operationalPreferences, isLoading: loadingPreferences } =
    useOperationalPreferences();
  ...
  const handleSavePreferences = () => {
    ...
    if (
      values.pending_sale_expiration_minutes < 5 ||
      values.pending_sale_expiration_minutes > 1440
    ) {
      toast.error("O tempo de expiração deve estar entre 5 minutos e 24 horas.");
      return;
    }
    if (
      values.temperature_aquecendo_sales_per_day <= 0 ||
      values.temperature_quente_sales_per_day <= values.temperature_aquecendo_sales_per_day ||
      values.temperature_explodindo_sales_per_day <= values.temperature_quente_sales_per_day
    ) {
  ...
  if (loadingOrg || loadingMp || loadingPreferences)
    return (
            ...
            {sections.map((section) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "flex items-center gap-3 rounded-[var(--radius-sm)] px-3 py-2 text-left text-body transition-colors",
                    isActive
                      ? "bg-[var(--accent-muted)] text-[var(--accent-text)] ring-1 ring-accent/20 dark:ring-accent/40"
            ...
            <Panel title="Organização">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  <div className="relative flex h-20 w-20 items-center justify-center overflow-hidden border border-[var(--border-default)] bg-[var(--bg-tertiary)]">
                    {isUploading ? (
              ...
              <div className={`border p-6 ${status.wrapper}`}>
                ...
                {mpStatus === "conectado" && mpConfig && (
                  <p className="mt-2 text-small text-[var(--text-secondary)]">
                    Ambiente ativo:{" "}
                    {(mpConfig as any).environment === "sandbox" ? "Sandbox" : "Produção"}
                  </p>
                )}
              </div>
              <Button asChild>
                <Link to="/admin/configuracoes/mercado-pago">
                  {mpStatus === "nao_configurado"
          ...
          {active === "backup" && (
            <Panel title="Backup de Dados">
              <p className="text-body text-[var(--text-secondary)]">
                A exportação inclui eventos, vendas, participantes, clientes e cortesias da sua
                organização.
              </p>
              ...
              <p className="text-small text-[var(--text-secondary)]">Recurso em desenvolvimento.</p>
            </Panel>
          )}
        </div>
      </div>
