...
export default function UsersListPage() {
  ...
  useAdminPageAction(
    <PrimaryActionButton onClick={() => setPanelOpen(true)}>Convidar Usuário</PrimaryActionButton>,
  );
            ...
            {isLoading ? (
              ...
              pageRows.map((user) => (
                <DataTableRow key={user.id}>
                  ...
                  <DataTableCell>
                    {user.invitedAt ? new Date(user.invitedAt).toLocaleDateString("pt-BR") : "—"}
                  </DataTableCell>
                  <DataTableCell>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[rgba(0,0,0,0.6)] p-6">
          <div className="w-full max-w-[400px] rounded-[var(--radius-lg)] bg-bg-primary p-6 shadow-[var(--shadow-lg)]">
            ...
            <p className="text-body text-text-secondary">
              O usuário <strong>{userToDelete.full_name || userToDelete.email}</strong> perderá o
